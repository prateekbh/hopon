    ...
    return (
      <div className={style.page}>
        ...
        <div className={style.buttonsDiv + ' ' +style.centerFlex}>
          <div>
            <Button raised={true} ripple={true}
                onClick={()=>{
            ...
            <Dialog ref={(dialog)=>{
              ...
              <Dialog.Footer>
                ...
                <Dialog.FooterButton primary={true} accept={true}
                  onClick={()=>{
                    if(this.state.userName.trim()){
                      localStorage.uname = this.state.userName.trim();
                      route('/play');
                    }
                  }}>OK</Dialog.FooterButton>
              </Dialog.Footer>
            </Dialog>
          </div>
          <div>
            <div className="inline-block">
              <Button className={style.googleLogin}
                  raised={true} ripple={true} primary={true}
                  disabled={this.state.firebaseLoading || this.state.isUserLoggingIn}
                  onClick={()=>{
                    this.loginWithGoogle();
                  }}> Login with Google</Button>
              {this.state.firebaseLoading && <LinearProgress indeterminate={true}/>}
            </div>
          </div>
        </div>
      </div>
